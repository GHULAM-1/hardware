...
type ItemsGridProps = {
  ...
  isDeleteDisabled: (row: ItemWithStock) => boolean;
        ...
        return (
          ...
          >
            ...
            <div className="relative aspect-[4/3] bg-gradient-to-br from-muted/60 to-muted/25">
              ...
              {/* At-a-glance "tracked in warehouse" chip. */}
              {item.track_in_warehouse && (
                <span
                  className="absolute bottom-1.5 start-1.5 inline-flex items-center gap-1 rounded-full bg-background/85 px-1.5 py-0.5 text-[10px] font-semibold text-foreground/80 shadow-sm backdrop-blur-sm"
                  title={t("items.tracked")}
                >
                ...
                <div className="absolute end-1.5 top-1.5" onClick={(e) => e.stopPropagation()}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      ...
                      >
                        {t("items.trackInWarehouse")}
                      </DropdownMenuCheckboxItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                        variant="destructive"
                        disabled={isDeleteDisabled(item)}
                        title={isDeleteDisabled(item) ? deleteDisabledReason : undefined}
